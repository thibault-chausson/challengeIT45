...
def main():
    ...
    pop_initiale = population_initiale.gen_n_solutions_uniques(nbPopInitiale)
    ...
    best_solution, best_fitness = pick_best(pop_resultante)
...
def pick_best(pop_resultante):
    distance = fitnessEmployes.distance_employe(activites_intervenants(pop_resultante[0]))
    best_fitness = fitnessSESSAD.fitnessSESSAD(MISSIONS, INTERVENANTS, pop_resultante[0], distance)
    best_solution = pop_resultante[0]
    for individu in pop_resultante:
        distance = fitnessEmployes.distance_employe(activites_intervenants(individu))
        fitness = fitnessSESSAD.fitnessSESSAD(MISSIONS, INTERVENANTS, individu, distance)
